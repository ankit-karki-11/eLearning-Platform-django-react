...
def recommend_courses(course_slug: str, top_n: int = 4):
    try:
        ...
        # 2. Combine relevant course info with weights
        df["combined_text"] = (
            df["title"].fillna("") + " " +
            df["keywords"].fillna("") + " " +  # Keywords likely very important
        ...
        # 5. Get index of the given course
        try:
            current_index = df[df["slug"] == course_slug].index[0]
        except IndexError:
            return Course.objects.none()

        # 6. Calculate similarity and get top N
        ...
        top_similar_ids = df.iloc[similar_indices]["id"].tolist()

        return Course.objects.filter(id__in=top_similar_ids)

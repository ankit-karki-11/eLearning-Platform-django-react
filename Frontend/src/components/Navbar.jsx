...
const Navbar = () => {
  ...
  // Render admin-specific navbar if user is admin
  if (user?.role === "admin") {
    return <AdminNavbar user={user} logoutHandler={logoutHandler} />;
  }

  // Regular user navbar (unchanged)
  return (
    <div className='h-14 w-full dark:bg-gray-900/95 bg-white backdrop-blur-sm fixed top-0 left-0 right-0 duration-300 z-50'>
      <div className='max-w-8xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-full'>
...
const AdminNavbar = ({ user, logoutHandler }) => {
  return (
    <div className='h-14 w-full dark:bg-gray-900/95 bg-white backdrop-blur-sm fixed top-0 left-0 right-0 duration-300 z-50'>
      <div className='max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-full'>
        <div className='flex items-center gap-6'>
          <Link to="/admin" className="flex items-center gap-2">
            <img src="/logo.png" alt="Digital Padhai Logo" className="h-6 w-auto" />
            <span className="font-semibold text-sm text-blue-500">Admin Panel</span>
          </Link>
        </div>
        ...
        <div className='flex items-center gap-4'>
          ...
          <div className='md:hidden'>
            <AdminMobileNavbar logoutHandler={logoutHandler} user={user} />
          </div>
        </div>
      </div>
    </div>
...
// Admin Mobile Navbar Component
const AdminMobileNavbar = ({ logoutHandler, user }) => {
  return (
    <Sheet>
        ...
        <nav className='flex flex-col space-y-2 mt-6'>
          {user && (
            <>
              ...
              <SheetClose asChild>
                <NavLink to="/admin/courses">Course Management</NavLink>
              </SheetClose>
              <SheetClose asChild>
                <NavLink to="/admin/enrollments">Enrollment Management</NavLink>
              </SheetClose>
...
// Search Form Component (unchanged)
const SearchForm = ({
  searchQuery,
  setSearchQuery,
  searchField,
...
// Mobile menu component for regular users (unchanged)

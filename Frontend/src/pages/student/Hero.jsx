    ...
    <div className='relative pt-24 pb-16 md:pt-32 md:pb-24 px-4 sm:px-6 lg:px-8 bg-white dark:bg-gray-950 overflow-hidden'>
      <div className='relative max-w-3xl mx-auto text-center'>
        <div className={`inline-flex items-center px-3 py-1.5 mb-6 rounded-full bg-gray-200 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 text-gray-900 dark:text-gray-200 text-xs tracking-wider transition-opacity duration-500 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
          <Sparkles className="mr-2 h-3 w-3 text-blue-500 dark:text-blue-400 uppercase" />
          Easy learning for everyone
        </div>

        <h1 className={`text-3xl sm:text-4xl md:text-4xl font-bold tracking-tight text-gray-900 dark:text-white mb-4 transition-opacity duration-500 delay-100 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
        ...
        <p className={`text-sm text-gray-600 mb-4 max-w-lg mx-auto leading-relaxed transition-opacity duration-500 delay-200 ${isVisible ? 'opacity-100' : 'opacity-0'}`}>
         Track your progress and strengthen your knowledge by practicing focused multiple-choice questions organized by topic and difficulty level.
        </p>
